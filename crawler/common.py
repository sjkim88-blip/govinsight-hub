...
KEYWORD_RULES = [
    ...
    (["대기업"], "#대기업참여가능", "role"),
    # ── 도메인 (6 카테고리) ────────────────────────────────────────────────
    # 산업별 뉴스 탭의 6개 도메인 서브탭과 1:1 대응(동향/KEIT 탭은 도메인이 아니라
    # src=='뉴스레터' 로 별도 분리). classify_news_domain() 도 이 규칙을 재사용한다.
    # 순서 = 매칭 우선순위(좁은 카테고리 먼저, 소재·에너지가 마지막 캐치올).
    (["철강", "포스코", "현대제철", "동국제강", "세아제강",
      "철근", "후판", "냉연", "열연", "고로", "제철"], "#철강", "domain"),
    (["AI", "인공지능", "피지컬AI", "피지컬", "로봇", "협동로봇",
      ...
      "원격교육", "교육데이터", "AI교육", "AI 교육"], "#AI", "domain"),
    (["반도체", "디스플레이", "시스템반도체", "웨이퍼", "전자", "가전",
      "파운드리", "팹리스", "OLED"], "#반도체·전자", "domain"),
    (["자동차", "전기차", "수소차", "자율주행", "모빌리티", "EV",
      "배터리", "이차전지"], "#모빌리티·자동차", "domain"),
    (["석유화학", "화학공정", "화학산업", "화학소재", "정유"], "#석유화학", "domain"),
    (["금속재료", "금속", "소재", "뿌리기술", "세라믹", "화합물",
      "섬유", "탄소나노", "탄소소재", "탄소중립", "탄소", "ESG", "수소",
      "신재생", "재생에너지", "태양광", "풍력",
      "조선", "방산", "해양", "항공", "공급망", "SCM", "물류",
      "뿌리산업", "도금", "주조", "단조", "용접", "표면처리"], "#소재·에너지", "domain"),
]

# 산업별 뉴스 탭 도메인 서브탭 6개(고정 순서). classify_news_domain() 반환값은
# 항상 이 중 하나. '동향(KEIT)' 은 도메인이 아니라 src=='뉴스레터' 로 별도 탭 처리.
NEWS_DOMAINS = ["철강", "AI", "반도체·전자", "모빌리티·자동차", "소재·에너지", "석유화학"]
